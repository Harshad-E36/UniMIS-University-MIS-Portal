...
class staff_master_aggregate(models.Model):
    College = models.ForeignKey(College, on_delete=models.CASCADE, related_name='staff_aggregates')
    Program = models.ForeignKey(CollegeProgram, on_delete=models.CASCADE, related_name='staff_aggregates') 
    Academic_Year = models.CharField(max_length=20)
...
class UserCollege(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} → {self.college.College_Code if self.college else 'Admin'}"